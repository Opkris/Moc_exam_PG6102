...
interface UserRepository : CrudRepository<User, String> {
    ...
    class UserService(
        ...
        fun buyMovieTicket(userId: String, movieId: String) {
            ...
            if (user.cash < price) {
                throw IllegalArgumentException("Not enough cash")
            }

            user.cash -= price

            addTicket(user, movieId)
        }

        private fun addTicket(user: User, movieId: String) {
            user.ownedMovieTickets.find { it.movieId == movieId }
                    ?.apply { numberOfVisitors++ }
                    ?: MovieTickets().apply {
                        this.movieId = movieId
                        this.user = user
                        this.numberOfVisitors = 1
                    }.also { user.ownedMovieTickets.add(it) }
